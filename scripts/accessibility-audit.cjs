...
(async () => {
  ...
  try {
    ...
    const exportPage = await browser.newPage({ viewport: { width: 390, height: 844 } });
    await exportPage.goto(`${baseUrl}/export.html?season=spring&period=day&weather=clear&scene=none&role=cover`, { waitUntil: "networkidle" });
    const exportAudit = await auditPage(exportPage);
    assert.deepEqual(exportAudit.unnamed, []);
    assert.deepEqual(exportAudit.focusFailures, []);
    assert.ok(exportAudit.width <= exportAudit.viewport + 1);
    assert.equal(await exportPage.locator(".export-slide-item").count(), 1);
