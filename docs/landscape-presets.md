...
既存のUI・背景レイヤー・保存形式を引き継ぎ、既存5景色に10種類の自然景色を追加。写真の輪郭は光と色として認識できる程度にぼかし、左側の文字領域を静かに保つ。宇宙だけは、成立している精細な星雲表現をそのまま保つ。
...
- Node標準テスト: 18件成功。追加景色のURL往復・部分プリセット適用・既存景色と混在する下書きの保存復元・20件のカタログ素材参照を含む。
- Chromium実画面: 20種類の選択、上下キーによる4段の移動、共有URL、再読み込み、6画面幅（1440 / 1280 / 900 / 768 / 390 / 320px）を確認。横にはみ出す選択肢なし、実行時エラーなし。
